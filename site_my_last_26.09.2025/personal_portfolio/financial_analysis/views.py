...
import logging
from .services import BalanceSheetAnalyzer, ProfitLossAnalyzer, CashFlowAnalyzer
from .parsers import extract_text_with_table_detection, parse_balance_sheet, parse_profit_loss, parse_cash_flow
...
def analyze_document(document):
    ...
    try:
        ...
        text = extract_text_with_table_detection(document.pdf_file.path)

        print(f"=== АНАЛИЗ ДОКУМЕНТА: {document.title} ===")
        print(f"Тип документа: {document.document_type}")
        ...
        if 'Бухгалтерский баланс' in text:
            document.document_type = 'balance'
            success = parse_balance_sheet(text, document)
            if success:
                analyzer = BalanceSheetAnalyzer(document)
                analyzer.save_ratios()

        elif 'Отчет о финансовых результатах' in text or 'прибылях и убытках' in text:
            document.document_type = 'profit_loss'
            success = parse_profit_loss(text, document)
            if success:
                analyzer = ProfitLossAnalyzer(document)
                analyzer.save_ratios()

        elif 'Отчет о движении денежных средств' in text:
            document.document_type = 'cash_flow'
            success = parse_cash_flow(text, document)
            if success:
                analyzer = CashFlowAnalyzer(document)
                analyzer.save_ratios()
